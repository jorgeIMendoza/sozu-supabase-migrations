-- Mediciones: usuarios_actividad_por_portal + dispositivo/marca/navegador
-- Fecha: 2026-07-11
--
-- Ampliación del drill-down por usuario (Fase 1 ya aplicada): agrega 3 columnas derivadas
-- del user_agent de la sesión MÁS reciente de cada usuario: tipo_dispositivo, marca_dispositivo
-- y navegador. Como cambia la firma de RETURNS TABLE (columnas de salida), hay que DROP antes
-- del CREATE (CREATE OR REPLACE no puede alterar las columnas de salida).
--
-- El front (MedicionesPortalesPage) ya lee estos campos y degrada a "—" si la RPC no está
-- actualizada, así que es seguro. Idempotente: DROP IF EXISTS + CREATE. Sin BEGIN/COMMIT.
--
-- App nativa SOZU: si el cliente registra la sesión con user_agent 'SozuApp/<ver> (<os>; <modelo>)'
-- (ver register_portal_session), se clasifica como tipo_dispositivo='app', navegador='App SOZU'
-- y la marca se deriva del os/modelo (ios→Apple, android→marca por token). Sin ese marcador el
-- UA de Flutter/Dart es genérico y caería en 'desktop'/'Otro'.
