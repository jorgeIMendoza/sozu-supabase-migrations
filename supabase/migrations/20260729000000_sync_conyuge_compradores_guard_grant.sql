-- =============================================================================
-- sync_conyuge_compradores — guard de autorización + GRANT a authenticated
--
-- Bug reportado: "Editar Comprador" (/admin/compradores) devuelve 403. El UPDATE
...
BEGIN
  ...
  -- Todos los roles con escritura en /admin/compradores pasan el guard por la
  -- rama de user_has_permission. Si alguno quedara fuera, el 403 se repetiría
  ...
  IF v_n < 9 THEN
    RAISE EXCEPTION 'Se esperaban >= 9 roles con escritura en /admin/compradores, hay %', v_n;
  END IF;
END
$$;
