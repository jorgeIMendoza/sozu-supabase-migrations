-- P30 — Cobranza: RPC única de detalle de cuenta (get_pcobranza_cuenta_detalle)
-- ---------------------------------------------------------------------------------
-- Homogeneiza el detalle (CC-XXXXXX): antes ~33 .from() en waterfall; ahora una sola RPC
-- devuelve TODO el detalle en jsonb con el MISMO shape (llaves camelCase) que consume el
-- front (fetchCuentaDetalle, ya reescrito). Catálogos, documentos y mutaciones (registrar
-- pago/multa, subir evidencia, eliminar) siguen directas/EF — no entran aquí.
--
-- Verificado vs esquema vivo (prod): todas las tablas/columnas/FKs existen
-- (categorias_producto.nombre, esquemas_pago.*, usuarios.rol_id, roles.nombre, etc.).
--
-- DESVIACIÓN vs SQL del plan (defensivo, sin cambio de comportamiento):
-- el SELECT INTO o se ejecuta SIEMPRE (antes iba dentro de IF v_oferta_id IS NOT NULL).
-- Motivo: `o` es RECORD; si v_oferta_id fuese NULL nunca se asignaba y `o.prop_id` abortaría
-- con "record o is not assigned yet". Ejecutándolo con v_oferta_id NULL → 0 filas → el record
-- queda asignado a NULLs. Hoy 0/1784 cuentas tienen oferta efectiva NULL, pero evita crash futuro.
