-- RPC get_proyecto_financials(p_proyecto_id): KPIs financieros por proyecto.
-- Fecha: 2026-06-18
--
-- Alimenta los cards superiores de "Relación de Pagos" (Portal Escrituración):
-- precio final principal, total pagado principal, saldo pendiente, efectivo pagado vs
-- límite, y valor de escrituración (incluye accesorios bodega/estacionamiento).
-- Solo lectura (STABLE). No modifica tablas ni toca get_relacion_pagos.
--
-- Verificado en dev: todas las tablas/columnas referenciadas existen; la función no
-- existía. Idempotente (CREATE OR REPLACE).

CREATE OR REPLACE FUNCTION public.get_proyecto_financials(p_proyecto_id bigint)
RETURNS TABLE (
...
COMMENT ON FUNCTION public.get_proyecto_financials(bigint) IS
  'Calcula KPIs financieros reales por proyecto para Relación de Pagos: precio final principal, total pagado principal, saldo pendiente, efectivo y valor de escrituración incluyendo accesorios.';
