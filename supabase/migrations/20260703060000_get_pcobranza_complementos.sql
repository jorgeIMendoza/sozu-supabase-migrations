-- Portal Cobranza > menú Complementos (antes "Productos y Mantenimiento").
-- Crea get_pcobranza_complementos: dashboard de cobranza de EXTRAS.
-- COMPLEMENTO EXACTO de get_pcobranza_inmuebles (que quedó solo con unidades del
-- flujo de venta: cuenta de unidad + conceptos 1-6). Aquí vive todo lo que aquel
-- EXCLUYE dentro de SOZU, sin traslape ni huecos (inmuebles + este = 100% SOZU):
...
-- Cuerpo idéntico a la interina get_pcobranza_productos_mantenimiento ya en dev;
-- solo cambia el nombre a get_pcobranza_complementos (el menú pasó a llamarse
-- Complementos). Universo: proyectos SOZU (entidades_relacionadas.id_tipo_entidad
-- = 5). La propiedad efectiva se resuelve por la cuenta o, si es hija, por su padre.
...
GRANT EXECUTE ON FUNCTION public.get_pcobranza_complementos(integer, integer[], text)
