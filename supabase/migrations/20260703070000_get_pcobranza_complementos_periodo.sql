-- Portal Cobranza > menú Complementos — agrega filtro de periodo (Año/Mes).
-- get_pcobranza_complementos suma p_fecha_inicio/p_fecha_fin (filtran por fecha de
-- vencimiento del cargo, ap.fecha_pago; NULL = todo el histórico). Esto CAMBIA la
-- firma de (integer, integer[], text) a (integer, integer[], text, date, date).
-- Se DROPEA el overload de 3 args (ya en dev via 20260703060000): con ambos
-- overloads presentes y todos los params con DEFAULT, una llamada de 3 args sería
-- ambigua. En prod el de 3 args puede no existir aún -> DROP ... IF EXISTS seguro.
-- Resto del cuerpo idéntico. CREATE OR REPLACE, SECURITY DEFINER. Sin BEGIN/COMMIT.

DROP FUNCTION IF EXISTS public.get_pcobranza_complementos(integer, integer[], text);
...
BEGIN
  ...
  WHERE ap.activo = true
    ...
    -- Periodo opcional (Año/Mes): filtra por fecha de vencimiento del cargo.
    AND (p_fecha_inicio IS NULL OR ap.fecha_pago >= p_fecha_inicio)
    AND (p_fecha_fin IS NULL OR ap.fecha_pago <= p_fecha_fin);
