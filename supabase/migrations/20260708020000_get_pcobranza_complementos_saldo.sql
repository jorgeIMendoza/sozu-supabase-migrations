-- Fix saldo/cartera — get_pcobranza_complementos (corrección de dinero).
--
-- Mismo modelo que Inmuebles: Saldo = Vencido + Por vencer (partición exacta de
-- lo no-completado). Cambios (CREATE OR REPLACE):
--   1) 'pendiente_total' pasa de TODO no-completado -> solo por-vencer
--      (fecha_pago >= hoy OR sin fecha).
--   2) por_proyecto.pendiente: mismo criterio.
--   vencido_total (SUM(pend) ... fecha_pago < hoy) NO cambia. por_categoria NO se
--   toca (ahí 'pendiente' sigue siendo el total no-completado por categoría).
--   Complementos no tiene contra-entrega (concepto 3 vive en Inmuebles).

CREATE OR REPLACE FUNCTION public.get_pcobranza_complementos(p_proyecto_id integer DEFAULT NULL::integer, p_entidad_ids integer[] DEFAULT NULL::integer[], p_tipo text DEFAULT NULL::text, p_fecha_inicio date DEFAULT NULL::date, p_fecha_fin date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'pendiente_total', (SELECT COALESCE(SUM(pend), 0) FROM _pm WHERE pago_completado = false AND (fecha_pago >= v_hoy OR fecha_pago IS NULL)),
