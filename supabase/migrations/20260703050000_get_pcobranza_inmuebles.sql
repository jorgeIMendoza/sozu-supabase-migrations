-- Portal Cobranza > menú Inmuebles (antes "Dashboard").
-- Crea get_pcobranza_inmuebles (convención get_p<portal>_<menu>), reemplazo de
-- get_dashboard_cobranza_kpis(integer, date, date, integer[]) — la versión
-- temp-table de 4 args que usa el front. Cuerpo idéntico a la interina
-- get_pcobranza_dashboard ya en dev; solo cambia el nombre a get_pcobranza_inmuebles
-- (el menú pasó a llamarse Inmuebles, URL /inmuebles; productos/mantenimiento
-- viven en el menú Complementos -> get_pcobranza_productos_mantenimiento).
-- Cubre solo unidades del flujo de venta (cuenta de unidad + conceptos 1-6):
-- deuda, aging, morosidad, series 5 años, pipeline (listas exige validación
-- 'coincide'), clientes_criticos (3+ parcialidades), ceps, duenos.
-- CREATE OR REPLACE, SECURITY DEFINER. Sin BEGIN/COMMIT (CI/CD envuelve en tx).
-- La interina get_pcobranza_dashboard y los overloads legacy get_dashboard_cobranza_kpis
-- se eliminan aparte, tras desplegar el front nuevo (ver Ejecuciones_manuales/portal-cobranza/dashboard.md).

CREATE OR REPLACE FUNCTION public.get_pcobranza_inmuebles(
