-- Fix saldo/cartera — get_pcobranza_inmuebles (corrección de dinero).
--
-- Modelo correcto (confirmado con negocio): el universo de deuda se parte en dos
-- cubetas que SUMAN exacto:
--   Vencido   = no-completados con fecha_pago < hoy (cualquier concepto, incl. contra-entrega).
--   Por vencer= no-completados con fecha_pago >= hoy O sin fecha.
--   Saldo     = Vencido + Por vencer = todo lo no-completado.
--   Cartera   = Cobrado + Saldo.
--
-- Cambios respecto a la versión previa (CREATE OR REPLACE):
--   1) 'pendiente_total' pasa de SUM(monto) futuro -> SUM(pend) por-vencer (incl. sin fecha).
--   2) por_proyecto.pendiente: mismo criterio (SUM(pend), fecha_pago >= hoy OR NULL).
--   vencido_total (SUM(pend) ... fecha_pago < hoy, todos los conceptos) NO cambia.
--   vencido_total_sin_ce se conserva (el front dejará de usarlo). por_cobrar_mes* intactos.

CREATE OR REPLACE FUNCTION public.get_pcobranza_inmuebles(p_proyecto_id integer DEFAULT NULL::integer, p_fecha_inicio date DEFAULT NULL::date, p_fecha_fin date DEFAULT NULL::date, p_entidad_ids integer[] DEFAULT NULL::integer[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'pendiente_total',       (SELECT COALESCE(SUM(pend), 0) FROM _ap WHERE pago_completado = false AND (fecha_pago >= v_hoy OR fecha_pago IS NULL)),
...
GRANT EXECUTE ON FUNCTION public.get_pcobranza_inmuebles(integer, date, date, integer[])
