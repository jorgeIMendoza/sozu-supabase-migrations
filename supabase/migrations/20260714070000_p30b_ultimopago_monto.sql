-- P30b — Cobranza: agrega `monto` a `ultimoPago` en get_pcobranza_cuenta_detalle
-- ---------------------------------------------------------------------------------
-- Único cambio vs P30 (20260714060000): el jsonb `up_json` (→ acuerdos[].ultimoPago)
-- ahora incluye 'monto' => pg.monto. Con esto la modal de carga de evidencia en CC
-- muestra el monto igual que RP. El resto del shape/comportamiento es idéntico.
--
-- IMPORTANTE: se conserva el fix defensivo de P30 (SELECT INTO o corre SIEMPRE,
-- fuera del IF v_oferta_id IS NOT NULL) para no reintroducir el crash
-- "record o is not assigned yet". CREATE OR REPLACE => idempotente.
