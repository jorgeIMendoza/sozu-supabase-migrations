...
-- Crea get_pcobranza_dashboard (convención get_p<portal>_<menu>), reemplazo de
-- get_dashboard_cobranza_kpis(integer, date, date, integer[]) — la versión
-- temp-table de 4 args que usa el front. Cambios vs. la vieja:
--   - Nombre a convención del portal.
--   - Serie mensual ampliada a 5 años (año actual + 4 previos) en cobrado_mensual
--     y programado_mensual (antes 12 meses), para que el selector de año del
--     dashboard muestre datos de años previos.
--   - UNIVERSO: TODOS los cálculos (montos, aging, morosidad, por_proyecto,
--     series mensuales, pipeline, clientes_criticos, ceps) ahora se limitan a
--     UNIDADES de proyectos SOZU (entidad relacionada tipo 5). Cuenta de unidad =
--     depto (sin producto) + Bodega(cat 2) + Estacionamiento(cat 1), que comparten
--     id_propiedad. Se EXCLUYEN condensadoras(4), paquetes/persianas(3) y servicios
--     (cat NULL) + proyectos ajenos. (Antes "Todos" contaba global: vendidas 8038.)
--     La propiedad efectiva se resuelve por la cuenta o, si es hija, por su padre.
--     Además la deuda y el cobrado se restringen a los CONCEPTOS del flujo de venta
--     (abonos al precio): Apartado(1), Enganche(2), Contra entrega(3), Pago
--     especial(4), Parcialidad(5), Cesión de derechos(6) — misma clasificación
--     "Durante obra + Entrega" del detalle de cuenta. Quedan fuera mantenimiento(11)
--     y conceptos no-venta (penalización, multa, devolución, reserva, etc.).
--     _pg (cobrado) es por aplicaciones_pago (no pagos.monto) para poder filtrar por
--     concepto/cuenta y quedar consistente con la deuda (_ap.pend).
--   - pipeline.listas_escrituracion: Vendidas (est 5) cuyo ÚNICO pago pendiente es
--     el contra entrega (concepto 3), mirando depto + bodega/estac (no productos).
--     Antes = saldo 100% liquidado + precio_final > 100000 (umbral eliminado).
--   - clientes_criticos: criterio = 3+ parcialidades vencidas (antes 90+ días
--     sin pagar). Se eliminó el campo dias_sin_pagar del payload (ya no se usa).
-- La vieja get_dashboard_cobranza_kpis (sus 3 overloads) se elimina aparte, tras
...
BEGIN
  ...
  result := result || jsonb_build_object('pipeline', (
    ...
    -- Cuentas de la UNIDAD: comparten id_propiedad con el depto — el depto
    -- (sin producto) + Bodega(cat 2) y Estacionamiento(cat 1). También cuentas
    -- hijas sin producto (via id_cuenta_cobranza_padre). Se EXCLUYEN condensadoras
    -- (4), paquetes/persianas (3) y servicios (cat NULL): van aparte, su deuda no
    -- afecta la disponibilidad para escriturar de la unidad.
    cuentas_prop AS (
      SELECT p.id AS prop_id, cc.id AS cuenta_id
      FROM cand p
      JOIN cuentas_cobranza cc
        ON cc.activo = true AND cc.id_tipo_cancelacion IS NULL
       AND (cc.id_propiedad = p.id
            OR cc.id_cuenta_cobranza_padre IN (
              SELECT m.id FROM cuentas_cobranza m
              WHERE m.id_propiedad = p.id AND m.activo = true AND m.id_tipo_cancelacion IS NULL))
      LEFT JOIN ofertas o ON o.id = cc.id_oferta
      LEFT JOIN productos_servicios ps ON ps.id = o.id_producto
      WHERE (o.id_producto IS NULL OR ps.id_categoria IN (1, 2))
    ),
    -- "Listas p/ escriturar" = Vendidas (est 5) cuyo ÚNICO pago pendiente es el
    -- pago final contra entrega (concepto 3), considerando depto + bodega/estac:
    -- cada cuenta de la unidad solo debe deber su contra entrega; nada de
    -- parcialidades/enganche/etc pendiente.
    listas AS (
      SELECT p.id FROM cand p
      WHERE EXISTS (SELECT 1 FROM cuentas_prop cp WHERE cp.prop_id = p.id)
        -- Ningún acuerdo pendiente que NO sea contra entrega (concepto 3),
        -- en la principal ni en ninguna dependencia.
        AND NOT EXISTS (
          SELECT 1 FROM cuentas_prop cp
          JOIN acuerdos_pago ap ON ap.id_cuenta_cobranza = cp.cuenta_id AND ap.activo = true
          WHERE cp.prop_id = p.id AND ap.pago_completado = false AND ap.id_concepto IN (1, 2, 4, 5, 6))
        -- Pero sí resta el pago final contra entrega (concepto 3) en alguna cuenta.
        AND EXISTS (
          SELECT 1 FROM cuentas_prop cp
          JOIN acuerdos_pago ap ON ap.id_cuenta_cobranza = cp.cuenta_id AND ap.activo = true
          WHERE cp.prop_id = p.id AND ap.pago_completado = false AND ap.id_concepto = 3)
    )
