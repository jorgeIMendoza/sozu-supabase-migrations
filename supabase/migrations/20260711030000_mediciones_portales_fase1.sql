-- Mediciones de uso de portales — Fase 1: tablas de tracking + RPCs + menús.
-- Fecha: 2026-07-11
--
-- Formaliza lo ya ejecutado a mano en PRD (Ejecuciones/ejecutar.md 2026-06-10):
-- sesiones y eventos por portal (12 portales), RPCs SECURITY DEFINER de
-- escritura (registrar/heartbeat/cerrar sesión, registrar evento) y de lectura
-- para los dashboards de Alta Dirección (online, históricos, mapa de calor de
-- menús y de CTAs), más los 3 submenús "Mediciones" del Portal Alta Dirección
-- con permisos para Super Admin.
--
-- Idempotente: tablas/índices con IF NOT EXISTS, funciones CREATE OR REPLACE,
-- inserts de menús con WHERE NOT EXISTS (seguro de re-ejecutar sobre PRD).
-- El RPC de drill-down por usuario (usuarios_actividad_por_portal) va en la
-- migración siguiente, ya en su versión final.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Tablas de tracking
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Sin policies: acceso sólo vía RPC SECURITY DEFINER.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. RPCs de escritura (front)
-- ─────────────────────────────────────────────────────────────────────────────

-- Idempotente: si hay sesión activa reciente (<30 min) del usuario en el
-- portal, la reutiliza como heartbeat; si no, crea una nueva.
...
-- Registrar evento (page_view / menu_click / submenu_click / cta_click);
-- cualquier evento cuenta como heartbeat de la sesión.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. RPCs de lectura (dashboards Alta Dirección)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. Submenús "Mediciones" en Portal Alta Dirección (Super Admin, rol 1)
-- ─────────────────────────────────────────────────────────────────────────────

-- 5.1 Submenús (id del menú resuelto por nombre, sin hardcodear).
...
FROM (
  ...
  WHERE nombre = 'Portal Alta Dirección' AND activo = true
  LIMIT 1
) m
CROSS JOIN (VALUES
...
-- 5.2 Permisos disponibles por submenú (1 = leer, 6 = exportar).
...
-- 5.3 Asignación a Super Admin (rol 1).
