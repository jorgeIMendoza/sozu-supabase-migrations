...
-- Quita el filtro Estatus propiedad de get_pcobranza_cuentas_cobranza:
-- antes se excluían Entregada (8) y Pagada completamente (9); ahora la RPC
-- devuelve TODO el universo de cuentas (cualquier id_estatus_disponibilidad).
-- El front decide qué mostrar con el filtro "Estatus" — así no se esconde data
-- y queda consistente con Relación de Pagos.
-- Único cambio vs 20260702030000: se elimina el bloque WHERE de id_estatus_disponibilidad.
...
BEGIN
  ...
  FROM (
    ...
    WHERE cc.activo = true
      ...
      -- Sin filtro de estatus: la RPC devuelve TODO el universo de cuentas
      -- (cualquier id_estatus_disponibilidad, incl. Entregada 8 y Pagada 9). El
      -- front decide qué mostrar con el filtro "Estatus" — así no se esconde data
      -- y queda consistente con Relación de Pagos.
      AND (
